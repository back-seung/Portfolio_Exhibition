	...
	<form action="userMod" method="post">
		<div class="inputForm col-md-4 offset-md-4 shadow-lg">
			<div class="mt-4 p-4">
				...
				<div>CURRENT ID : ${user.id}</div>
				<div>CURRENT NAME : ${user.name}</div>
				<hr>
				<div class="text-center">
					<div class="form-floating mb-3 nameForm">
						<input type="hidden" name="id" value="${user.id}" /> <input
							type="text" class="form-control" id="floatingInput" name="name"
							placeholder="변경하고자 하는 이름을 입력해주세요"> <label
							for="floatingInput">변경할 이름</label>
					</div>
					<input class="btn btn-secondary mt-3" type="submit" value="수정완료" />
				</div>
			</div>
		</div>
	</form>
